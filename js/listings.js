...
/* ── Paltar kateqoriyaları ── */
const LISTING_CATEGORIES = [
  { value: 'donlar',      label: '👗 Donlar & Ətəklər' },
  { value: 'ust_geyim',   label: '👚 Üst geyim (köynək, bluz, t-shirt)' },
  { value: 'alt_geyim',   label: '👖 Alt geyim (şalvar, jeans)' },
  { value: 'godekcə',     label: '🧥 Gödəkçə & Palto' },
  { value: 'kazak',       label: '🧶 Kazak & Sviter & Hoodie' },
  { value: 'kostyum',     label: '🤵 Kostyum & Blazer' },
  { value: 'idman',       label: '🏃 İdman geyimi' },
  { value: 'yay',         label: '☀️ Yay geyimi & Plaj' },
  { value: 'alt_paltar',  label: '🌙 Alt paltarı & Pijiama' },
  { value: 'cocuq',       label: '👶 Uşaq geyimi' },
  { value: 'aksesuarlar', label: '🎒 Aksesuarlar & Çantalar' },
  { value: 'diger',       label: '📦 Digər' }
];

/* ── Preset rənglər ── */
const PRESET_COLORS = [
  { name: 'Qara',         hex: '#111111' },
  { name: 'Ağ',           hex: '#FFFFFF' },
  { name: 'Boz',          hex: '#9CA3AF' },
  { name: 'Tünd boz',     hex: '#4B5563' },
  { name: 'Krem',         hex: '#FEF3C7' },
  { name: 'Bej',          hex: '#D4B896' },
  { name: 'Qəhvəyi',      hex: '#92400E' },
  { name: 'Karamel',      hex: '#B45309' },
  { name: 'Tünd mavi',    hex: '#1E3A5F' },
  ...
  { name: 'Açıq çəhrayı', hex: '#FBCFE8' },
  { name: 'Narıncı',      hex: '#F97316' },
  { name: 'Sarı',         hex: '#EAB308' },
  { name: 'Limon',        hex: '#D9F99D' },
  { name: 'Yaşıl',        hex: '#16A34A' },
  { name: 'Zeytun',       hex: '#65A30D' },
  { name: 'Tünd yaşıl',   hex: '#14532D' },
...
function renderListingCard(l) {
  ...
  const catLabel   = (LISTING_CATEGORIES.find(c => c.value === l.category) || {}).label || l.category || '';
  ...
  <div class="lac-card" data-id="${l.id}">
    <div class="lac-img-wrap">
      ${img
        ...
        : `<div class="lac-img-placeholder">👗</div>`}
    ...
    <div class="lm-modal" id="listingFormModal">
      ...
      <div class="lm-body">
        ...
        <div class="lm-row2">
          ...
          <div class="lm-field">
            <label class="lm-label">Kateqoriya <span style="color:var(--danger)">*</span></label>
            <select class="lm-input lm-select" id="lmCategory">
              <option value="">Seçin...</option>
              ${LISTING_CATEGORIES.map(c => `<option value="${c.value}">${c.label}</option>`).join('')}
            </select>
          </div>
        </div>
        ...
        <div class="lm-section">
          ...
          <div id="lmColorPicker" class="lm-color-picker" style="display:none;">
            <div class="lm-cp-presets">
              ${PRESET_COLORS.map(c => `
                <button type="button" class="lm-cp-swatch" data-hex="${c.hex}" data-name="${c.name}"
            ...
            <div class="lm-cp-custom">
              <div class="lm-cp-custom-preview" id="lmCpPreview" style="background:#E5E7EB;"></div>
              ...
              <input type="text" class="lm-input lm-cp-hex" id="lmCpHexText" placeholder="#Rəng kodu"
                maxlength="7"
...
async function openListingModal(editId = null) {
  ...
  document.getElementById('lmCategory').value  = '';
  ...
  // Edit modunda məlumatları yüklə
  if (editId) {
    ...
    if (doc.exists) {
      ...
      document.getElementById('lmCategory').value  = d.category  || '';
...
function closeListingModal() {
  document.getElementById('listingFormOverlay').classList.remove('lm-open');
}

function closeLMIfBg(e) {
  if (e.target.id === 'listingFormOverlay') closeListingModal();
}

/* ── Mağaza adını Firestore-dan doldur ── */
...
function lmRenderImages() {
  ...
  if (lState.images.length < 5) {
    ...
    add.innerHTML  = `
      ...
      <span>${lState.images.length}/5</span>`;
    wrap.appendChild(add);
  }
}

function lmRemoveImg(i) {
  lState.images.splice(i, 1);
  lmRenderImages();
}
...
function lmAddColor(name, hex) {
  if (lState.colors.some(c => c.hex === hex)) {
    showToast('Bu rəng artıq əlavə edilib');
    return;
  }
...
function lmRemoveColor(i) {
  lState.colors.splice(i, 1);
  lmRenderColors();
}
...
function lmAddSize() {
  ...
  if (lState.sizes.some(s => s.label.toLowerCase() === label.toLowerCase())) {
    showToast('Bu ölçü artıq var'); return;
  }
...
function lmRemoveSize(i) {
  lState.sizes.splice(i, 1);
  lmRenderSizes();
}

function lmUpdateSizeStock(i, val) {
  lState.sizes[i].stock = parseInt(val) || 0;
}
...
function lmStepStock(i, delta) {
  const newVal = Math.max(0, (lState.sizes[i].stock || 0) + delta);
  lState.sizes[i].stock = newVal;
  lmRenderSizes();
}
...
function closeStockModal() {
  ...
  stockEditId = null; stockEditData = null;
}

function closeStockIfBg(e) {
  if (e.target.id === 'stockModalOverlay') closeStockModal();
}

/* ══════════════════════════════════════════════
   STİLLƏR — dinamik inject
   ══════════════════════════════════════════════ */
function injectListingStyles() {
  ...
  .lac-active { background:#dcfce7; color:#166534; }
  .lac-out    { background:#fee2e2; color:#991b1b; }
  ...
  .lac-price      { font-size:.95rem; font-weight:700; }
  .lac-sale       { color:#dc2626; }
  ...
  .lac-btn-edit   { background:#eff6ff; color:#1d4ed8; }
  .lac-btn-edit:hover { background:#dbeafe; }
  .lac-btn-stock  { background:#f0fdf4; color:#166534; }
  .lac-btn-stock:hover { background:#dcfce7; }
  .lac-btn-delete { background:#fef2f2; color:#dc2626; margin-left:auto; }
  .lac-btn-delete:hover { background:#fee2e2; }
  ...
  .lm-input {
    ...
    font-size:.875rem; color:var(--accent); background:var(--bg);
  ...
  .lm-img-add {
    width:76px; height:76px; border:1.5px dashed #ccc;
    border-radius:10px; display:flex; flex-direction:column;
    align-items:center; justify-content:center; gap:4px;
    cursor:pointer; color:#aaa; font-size:.7rem;
  ...
  .lm-size-name-in  { width:140px; }
  .lm-size-stock-in { width:90px; }
  ...
  .lm-stock-input {
    width:64px; text-align:center; padding:.4rem .5rem;
    border:1.5px solid var(--border); border-radius:6px;
    font-size:.875rem; font-weight:600; outline:none;
  }
  ...
  .lm-btn-primary { background:var(--accent,#1a1a1a); color:#fff; border:none; }
  .lm-btn-primary:hover { opacity:.85; }
  .lm-btn-outline { background:none; border:1.5px solid var(--border); color:var(--muted); }
  .lm-btn-outline:hover { border-color:var(--accent); color:var(--accent); }
  ...
  .lm-btn-submit:hover { opacity:.88; }
  .lm-btn-submit:disabled { opacity:.5; cursor:not-allowed; }
